...
$_POST['markr'] = preg_replace("`http://([^\"'<>\r\n\s]+)`i", "<a target=\"_blank\" href=\"http://$1\">http://$1</a>", str_replace("<","&lt;",str_replace("\x1b","",str_replace("\n","",str_replace("\r","",str_replace("'","\"",$_POST['markr']))))));
fputs($jdmk,$_SESSION['m_nick']."\x1b".$_POST['markr']."\n");
...
if($i >= $start && $i < $end) {
$dkm = explode("\x1b",$dmko);
echo "<b>{$dkm[0]}</b> :: <font color='#030C1A'>{$dkm[1]}</font><br />";
}
$i++;
}
fclose($dmk);
...
$mkfile = "widget/marker.dat";
$dmk = fopen($mkfile,"r");
for($kk = -1;$kk <= 10 && ($dmko = trim(fgets($dmk)));$kk++) {
$mko = explode("\x1b",$dmko);
echo "<b>{$mko[0]}</b> :: {$mko[1]}<br />";
}
fclose($dmk);
$kk = $kk*20;
?></div></div>
<input type='button' value='목록' onclick='popup("widget/marker.php?page=1",600,360)' class='srbt' style='float:right;width:50px' />
<? if($mbr_level){?>
...
$('markr').value = '';
$('marker').firstChild.innerHTML = "<b><?=$_SESSION['m_nick']?></b> :: " + nkfval + "<br />" + $('marker').firstChild.innerHTML;
clearTimeout(changemk);
chmarker(1);
$('wmark').style.display="none";
startax("widget/marker.php?&markr=" + encodeURIComponent(nkfval));
}
function wmk_open() {
var wmrt = $('wmark');
var mrkr = $('markr');
